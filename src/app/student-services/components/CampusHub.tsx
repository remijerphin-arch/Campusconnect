  ...
  return (
    <section className="rounded-[2rem] border bg-card p-6 shadow-card">
      ...
      {activeTab === 'today' && <div className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <Card><p className="text-sm font-semibold text-primary">Focus for placement</p><p className="mt-2 text-3xl font-bold">{Math.min(100, 35 + career.mockInterviews * 10 + career.aptitudeSessions * 8 + (career.resumeReady ? 25 : 0))}%</p><p className="mt-1 text-sm text-muted-foreground">Readiness score based on your completed preparation.</p><div className="mt-4 flex gap-2"><button type="button" onClick={() => setActiveTab('career')} className="rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground">Continue preparing</button><button type="button" onClick={() => toast.success('Daily study checklist synced with schedule')} className="rounded-xl border px-3 py-2 text-sm font-semibold hover:bg-muted">Sync goals</button></div></Card>
      </div>}
